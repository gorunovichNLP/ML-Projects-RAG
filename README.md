...
Небольшой размеченный набор находится в `data/evaluation.jsonl`. Для каждого
вопроса в нём явно указаны допустимые `relevant_chunk_ids`.
...
Если стоит `—`, правильный чанк не попал в соответствующий top-20.
Итоговые метрики также сохраняются в `data/evaluation_metrics.json`; каждый
новый запуск полностью обновляет этот файл.
